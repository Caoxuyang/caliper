...
async function test() {
    ...
    method = web3.eth.sendSignedTransaction.method;
    var accountA = web3.eth.accounts.create(url);
    var accountB = web3.eth.accounts.create(url);
    var str_pri = "" + accountA.privateKey;
    var privateKeyA = new Buffer(str_pri.substring(2), 'hex');
    //var privateKeyB = new Buffer(accountB.privateKey.subString(2), 'hex');
    //var addressA = accountA.address;
    var addressB = accountB.address;
    ...
    for(var i=0;i<10;i++){
        mySendSignedTxAsync(signedTxs.pop());
    }
